...
def main():
    ...
    ap.add_argument("input", nargs="?", default="data_conv/data/train.jsonl", help="输入 messages jsonl 路径")
    ap.add_argument("-o", "--output", default="data_conv/data/train_dpo.jsonl", help="输出 DPO jsonl 路径")
    ...
    data_dir = os.path.join(root_dir, "data_conv", "data")

    if args.merge:
        # 合并 data_conv/data 下所有 train.jsonl
        inputs = [os.path.join(data_dir, "train.jsonl")]
        extra = [
            os.path.join(data_dir, "datagirl_train.jsonl"),
            os.path.join(data_dir, "music", "train.jsonl"),
            os.path.join(data_dir, "film", "train.jsonl"),
            os.path.join(data_dir, "travel", "train.jsonl"),
        ]
        for p in extra:
            if os.path.isfile(p):
                inputs.append(p)
