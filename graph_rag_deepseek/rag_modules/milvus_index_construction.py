...
class MilvusIndexConstructionModule:
    ...
    def __init__(self, 
                 # 默认走 IPv4 回环，避免 localhost 在部分环境优先解析到 ::1（IPv6）造成连接异常
                 host: str = "127.0.0.1", 
                 port: int = 19530,
    ...
    def _setup_client(self):
        """初始化Milvus客户端。pymilvus 要求 uri 以 [http, https, tcp, unix] 开头；经 SSH 隧道时需较长超时。"""
        # 经 SSH 反向转发时握手较慢，给 60 秒超时
        timeout = 60.0
        last_err: Optional[Exception] = None
        for uri in (f"http://{self.host}:{self.port}", f"tcp://{self.host}:{self.port}"):
            try:
                self.client = MilvusClient(uri=uri, timeout=timeout)
                collections = self.client.list_collections()
                logger.info(f"已连接到Milvus服务器: {uri}，当前集合: {collections}")
                return
            except Exception as e:
                last_err = e
                logger.warning(f"Milvus 连接尝试失败（{uri}）: {e}")
                continue
